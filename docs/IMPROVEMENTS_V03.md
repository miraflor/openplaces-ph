...
It does not widen the normalized schema with source-native fields. That is the
next data-model change for PSIC/PCPC/PSCC work, and it changes cache and output
schemas, so it deserves its own migration and tests.
